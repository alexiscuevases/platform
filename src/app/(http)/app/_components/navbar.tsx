  ...
  return (
    <div className="mx-auto flex max-w-[1240px] items-center justify-between">
      ...
      <div className="flex items-center space-x-6">
        <Link href={`/${business._id}/orders`} className="group relative font-semibold text-primary-dark">
          Pedidos
          <div
            className={`absolute -bottom-0.5 left-0 right-0 h-[1px] ${
              thisPage === "orders" ? "w-full" : "w-0 duration-200 group-hover:w-full"
            } bg-primary-dark`}
          />
        </Link>
        <Link href={`/${business._id}/products`} className="group relative font-semibold text-primary-dark">
          Productos
          <div
            className={`absolute -bottom-0.5 left-0 right-0 h-[1px] ${
              thisPage === "products" ? "w-full" : "w-0 duration-200 group-hover:w-full"
            } bg-primary-dark`}
          />
        </Link>
        <Link href={`/${business._id}/customers`} className="group relative font-semibold text-primary-dark">
          Clientes
          <div
            className={`absolute -bottom-0.5 left-0 right-0 h-[1px] ${
              thisPage === "customers" ? "w-full" : "w-0 duration-200 group-hover:w-full"
            } bg-primary-dark`}
          />
        </Link>
        <Link href={`/${business._id}/analytics`} className="group relative font-semibold text-primary-dark">
          Analítica
          <div
            className={`absolute -bottom-0.5 left-0 right-0 h-[1px] ${
              thisPage === "analytics" ? "w-full" : "w-0 duration-200 group-hover:w-full"
            } bg-primary-dark`}
          />
        </Link>
        <Link href={`/${business._id}/configuration`} className="group relative font-semibold text-primary-dark">
          Configuración
          <div
            className={`absolute -bottom-0.5 left-0 right-0 h-[1px] ${
              thisPage === "configuration" ? "w-full" : "w-0 duration-200 group-hover:w-full"
            } bg-primary-dark`}
          />
        </Link>
      </div>
      <button className="flex items-center space-x-2">
        <span className="flex h-8 w-8 items-center justify-center rounded-full bg-primary text-sm font-light text-white">
          AC
        </span>
        <span className="font-medium">Alexis Cuevas</span>
      </button>
    </div>
  );
}
